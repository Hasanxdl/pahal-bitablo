...
if (empty($_SESSION['user'])) {
  ...
  $duzenlenecek_blog  = strip_tags(@$_GET['id']);

  $gelenvar=$connn->prepare("SELECT * FROM blog_konulari WHERE blgkonu_id=$duzenlenecek_blog");
  $gelenvar->execute();

  $gelen_ayr=$gelenvar->fetch(PDO::FETCH_ASSOC);
}
...
<div class="card">
              ...
              <br>
              <div class="card-body">
                <form action="gorev_duzenle2.php" method="post">

                  <div class="form-group">
                    <label class="control-label">Yazı Başlığı</label>
                    <input type="text" name="Blogbaslik" placeholder="Lütfen Başlık Belirtiniz.." class="form-control" value="<?php echo $gelen_ayr['blgkonu_baslik']; ?>">
                  </div>
                  <br>

                  <div class="form-group">
                    <label class="control-label">Yazı İçeriği</label>
                    <textarea rows="4" name="Blogicerik" placeholder="Lütfen Açıklama Yapınız.." class="ckeditor"><?php echo $gelen_ayr['blgkonu_icerik']; ?></textarea>
                  </div>
                  <br>

                   <div class="form-group">
                    <label class="control-label">Resim Linki</label>
                    <input type="text" name="Blogresim" placeholder="Lütfen Başlık Belirtiniz.." class="form-control" value="<?php echo $gelen_ayr['blgkonu_resim']; ?>">
                  </div>
                  <br>

                  <div class="form-group">
                    <label class="control-label">Etiketler</label>
                    <input type="text" name="Blogetiket" placeholder="Lütfen Başlık Belirtiniz.." class="form-control" value="<?php echo $gelen_ayr['blgkonu_etiket']; ?>">
                  </div>
                  <br>

                  <div class="form-group">
                    <label class="control-label">Kategori No</label>
                    <input type="text" name="Blogkategori" placeholder="Lütfen Başlık Belirtiniz.." class="form-control" value="<?php echo $gelen_ayr['blgkonu_kategori']; ?>">
                  </div>
                  <br>

                   <div class="form-group">
                    <label class="control-label">Yazar Kim ?</label>
                    <input type="text" name="Blogyazari" placeholder="Lütfen Başlık Belirtiniz.." class="form-control" value="<?php echo $gelen_ayr['blgkonu_yazan']; ?>">
                  </div>                  
                
              </div>
              <div class="card-footer">
<input type="hidden" name="DuzenleTarihi" value="<?php echo date('Y-m-d'); ?>">
<input type="hidden" name="id_degeri" value="<?php echo $duzenlenecek_blog; ?>">                 
